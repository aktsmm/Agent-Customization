...
| デフォルトが存在しない場合  | C:\03_GITHUB\_output-session-dialogue\（フォールバック） |
| 「workspace」「ローカル」等 | `{workspace}/_output-session-dialogue/`                   |
| パス指定あり                | 指定パス                                         |

- ファイル名: `YYYYMMDD-NN--{topic-slug}.md`（NN = 同日連番 01, 02, 03...）
  - 出力先に同日ファイルが既にあれば次の番号を採番する
