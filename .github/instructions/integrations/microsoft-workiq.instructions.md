...
- 複数論点を 1 回のクエリにまとめない。1 論点 1 照会にし、日付、会議名、顧客名で分割する。まとめると各項目が浅くなり、**実際には完了している事項を「証跡なし」と誤判定する**（2026-09 実測: 持ち帰り 4 件を一括照会して「未送付」と記録 → 個別照会で送付済みと判明）。
- 検索結果に出ないことを「未実施」の証拠にしない。`未確認` と記録し、完了 / 未完了を断定しない。Teams 本文は `Expired` で本文を復元できないことがある。
