...
> 詳細は [git.instructions.md](../dev/git.instructions.md) を参照してください。

- **`git push` の禁止**: ユーザーの明示的な指示がない限り、リモートへのプッシュは禁止
- **`--force` の禁止**: 強制プッシュは原則禁止
- **ブランチ保護**: `main` / `master` への直接コミットは避ける
