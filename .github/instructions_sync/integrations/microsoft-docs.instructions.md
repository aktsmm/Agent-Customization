...
ドキュメント検索以外のツール活用ガイドラインです。

### 9.1 Web ページ取得

| ツール           | 用途                 | 備考                                         |
| ---------------- | -------------------- | -------------------------------------------- |
| `fetch_webpage`  | Web ページの内容取得 | 推奨（日本語対応）                           |
| `curl -sL <URL>` | フォールバック       | `fetch_webpage` がリダイレクトで失敗した場合 |
| `curl -sI <URL>` | HTTP ステータス確認  | ヘッダーのみ取得                             |

> 注意: `curl -sL` は日本語が文字化けする場合があるため、`fetch_webpage` を優先してください。

### 9.2 Microsoft 365 データ検索

`mcp_workiq_ask_work_iq` ツールで M365 全体を検索できます。

| データ種別     | クエリ例                                                 |
| -------------- | -------------------------------------------------------- |
| **メール**     | 「○○さんからのメールを探して」「今週の未読メールは？」   |
| **会議・予定** | 「今日の会議を教えて」「来週のスケジュールは？」         |
| **ファイル**   | 「最近編集したファイルを探して」「Q4 計画の Excel は？」 |
| **Teams**      | 「○○チャンネルのメッセージをまとめて」                   |
| **SharePoint** | 「○○サイトのドキュメントを探して」                       |
| **人**         | 「○○プロジェクトに関わってる人は？」                     |
