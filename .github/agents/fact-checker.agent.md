...
5. 製品名やブランド名が含まれる場合は、最新の overview / what-is / product page を 1 本確認し、旧称・改称・classic/new の有無を切り分ける
6. Brave で不足する場合だけ、`runInTerminal` で read-only の HTTP 取得を補完する
7. 重要な主張から順に、`Verified`、`Unverified`、`Contradicted` に分類する
8. 根拠の弱いもの、古そうなもの、食い違うものを優先して深掘りする
9. 影響の大きい問題から順に返す
