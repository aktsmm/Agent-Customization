...
description: "プロダクトを機能・非機能・UI/UX で徹底レビューし、修正・テスト・同観点スイープ・ドキュメント整備・不要物削除まで自律実行。Run Ledger / Handoff Packet で反復改善を継続。明示 release mode では品質 gate 後に配布と公開状態確認まで進める"
...
前回 state は、直近会話の `Run Ledger` / `Handoff Packet`、ユーザー指定の state file、既存の `.github/refine-product.md` の順に読む。

`.github/refine-product.md` が無い場合は既定では作らない。永続 state の作成確認は、ファイル変更を行う mode（`confirm` / `standard` / `quick` / `release`）だけで行う。`plan only` / `review only` では作成確認を省略する。

未回答、`auto` mode、Target/Focus `all` では作らない。Target/Focus `all` は包括的な一回実行寄りなので、永続 state は scoped な反復改善 run に限って使う。

新規作成する場合は、同時に `.gitignore` へ `/.github/refine-product.md` を追加し、agent state を Git 追跡対象にしない。既に追跡済みなら削除や `git rm --cached` は自動実行せず、ユーザー確認を取る。

`.github/refine-product.md` は、既存 file がある場合、またはユーザーが作成/更新を明示した場合だけ更新する。内容は last run、target、Open Items、verification、Do Not に絞り、secret、個人データ、ローカル絶対パス、一時ログ、網羅的 Findings は書かない。
...
Prior State Used: state file (.github/refine-product.md)|previous Handoff Packet|previous Run Ledger|none
...
Prior State Used: {Run Ledger / Handoff Packet / state file (.github/refine-product.md) / none}
...
- `.github/refine-product.md` の作成/更新は、既存 file またはユーザー明示に限った。
