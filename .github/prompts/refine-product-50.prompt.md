...
description: "日常コードレビュー向け軽量リファインプロンプト。Use when: refine product 50, code review, レビュー, 軽量修正。既定では安全な Fix now を最小修正・同根 Sweep・検証まで行い、plan/review/dry-run では編集しない"
argument-hint: "対象ファイル/差分/PR、重点観点、モード（quick / plan / review / dry-run）"
agent: "agent"
---
...
<!-- pattern: Lightweight evaluator-optimizer (review -> fix/guard -> sweep -> verify) -->

# refine product 50

日常コードレビュー向けの軽量リファインプロンプト。対象コードをレビューし、安全な `Fix now` は最小修正、同根 Sweep、検証まで 1 サイクルで閉じる。
...
- Mode: 空欄 / `fix` / `improve` / `quick` / `plan` / `review` / `dry-run`。空欄は default

| Mode | 動作 |
| --- | --- |
| default / `fix` / `improve` | 安全な `Fix now` を最小修正し、同根 Sweep・検証まで行う |
| `quick` | P0/P1 と近傍検証を優先し、関連する最小 Sweep / Guard まで行う |
...
- No-Edit mode 以外では、`Fix now` をレビューだけで残さない。安全に編集できない場合は `Guard now` 成果物または `Block` にする。
- No-Edit mode では、修正、state file 作成/更新、永続メモ更新をしない。
- Release、publish、version sync、配布物 hygiene は扱わない。必要なら `refine-product-100` に切り替える。
- `.git/info/`、`.github/` などにこの prompt 専用の state file は作らない。

## Workflow

```text
Context -> Review -> Fix/Guard -> Sweep -> Verify -> Report
```
...
No-Edit mode（`plan` / `review` / `dry-run`）では、Findings / Plan / Next Steps を出して終了する。

実行 mode（default / `fix` / `improve` / `quick`）では、安全な `Fix now` を修正する。`Guard now` はテスト、静的チェック、docs、確認手順のいずれか 1 つ以上を成果物にする。`Block` は実行しない。
...
- 修正した観点の Sweep を行った。
- 検証結果またはスキップ理由が具体的。
- release / publish / dashboard / state file 更新を始めていない。
- 学びは候補提示に留め、永続化していない。
