...
- Frontmatter Hygiene: `.instructions.md` / `.prompt.md` / `.agent.md` の `description` 欠落、必要な `applyTo` 欠落、過剰スコープ（手動参照用なのに `**`）、クォート不統一、`prompts-metadata.instructions.md` で定義した `syncToGlobal` / 著者メタの欠落
