...
description: "workspace の .github/skills に SKILL 知見やローカル修正を反映し、必要に応じて private skill repo へ mirror する。Use when: workspace skill, local skill fix, skill retro, SKILL 知見反映, private mirror"
...
セッションやインシデントから再利用可能な知見を抽出し、現在の workspace の `.github/skills/<skill>/` へ統合する。workspace で作成・修正した skill を private skill repo に移したい場合もこの prompt で扱う。指定がある場合だけ private skill repo にも同じ変更を mirror する。公開同期、commit、push はしない。
...
- 使う: ローカル workspace で作った、または修正した skill を private skill repo に移す準備をしたいとき
...
| public skill repo | 対象外。private mirror 後に `sync-public-skills` に handoff |
...
- private mirror でも commit / push はしない。公開同期する場合は、mirror 後に対象 skill 名付きで `sync-public-skills` に渡す
...
- private mirror 済みで公開同期する場合: `sync-public-skills` を対象 skill 名つきで実行し、private repo 側で commit / push / public sync を行う
```
