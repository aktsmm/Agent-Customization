...
この prompt 自体は public-safe な placeholder を保持する。
この環境で検索を省略したい場合だけ、以下の環境変数を使って local path を上書きしてよい。
...
環境変数はローカル専用 override として扱い、prompt 本文には実パスを埋め戻さない。
...
- private repo に他の未コミット skill がある場合、公開安全で今回の主題と矛盾しないものは **co-batch candidate** として一緒に扱ってよい
- co-batch は `skill 単位` で判断する。skill 配下の変更が公開安全か、差分意図が説明できるか、既存の blacklist と矛盾しないかを確認する
- dirty な他 skill があっても、`git add -A` で repo 全体を機械的に巻き込まない。対象 skill を明示して stage / commit / push する
- 公開不可または判断保留の dirty skill が混在する場合は、その skill を除外して続行する。unsafe skill を理由に safe skill 全体まで止めない
...
- まず `SYNC_PUBLIC_SKILLS_PRIVATE_REPO` / `SYNC_PUBLIC_SKILLS_PUBLIC_REPO` / `SYNC_PUBLIC_SKILLS_SCRIPT` があればそれを優先して使う
