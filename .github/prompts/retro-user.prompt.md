...
知見抽出に入る前に、サブエージェントで User Data の反映先候補を inventory する。どの `.prompt.md` / `.instructions.md` / `.agent.md` が受け皿になりそうか、既存ルールと重複しないかを先に見る。これで always-on ファイルへの不要な追加や、同じ知見の複数ファイル記載を防ぐ。
