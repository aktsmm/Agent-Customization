...
description: "VS Code workspace の `.github/skills` skill を private skill repo へ取り込み・育成し、既存 SKILL / references への知見反映と safe-auto の local commit まで行う。Use when: private skill retro, skill repo authoring, private skill fix, workspace skill intake, retro private skills"
...
セッションやインシデント、または VS Code workspace の skill から再利用可能な知見を抽出し、private skill repo の既存 skill へ最小差分で統合する。safe-auto では authoring 後に local commit まで行うが、remote push や public sync は行わない。
...
- safe-auto で変更した場合は、local commit 作成後に private repo の working tree が clean であることを確認する
