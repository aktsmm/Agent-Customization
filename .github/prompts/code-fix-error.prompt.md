...
- 状態ブロック: `.github/review-learnings.md` 内の `prompt-state:code-fix-error`
- ダッシュボード: `DASHBOARD.md`（なければ作成）
- 実行開始時に state block を読んで、前回 `Not Done` / `Next Steps` を優先観点にする
- 実行終了時は `prompt-state:code-fix-error` ブロックだけ上書きする
- `Act > 新観点` には前回 Carry Over と異なる観点を最低 1 件含める
