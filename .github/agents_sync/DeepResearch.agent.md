...
tools:
  ['read/readFile', 'edit/editFiles', 'search', 'web', 'brave-search/*', 'microsoftdocs/*', 'workiq/*', 'agent', 'todo']
---
...
3. 観点ごとに、#tool:runSubagent で調査サブエージェントを実行する。
   - **検索戦略**: 広く始めて絞り込み、再帰的に深掘りする
   - **再帰的収集**: 発見したリンクをたどり、元ソースや関連仕様を収集（最大3階層）
...
- **検索戦略**:
  ...
  3. **再帰的収集**: 記事内リンクをたどり、元ソースや関連仕様を深掘り
  4. **横断調査**: 複数の標準仕様・ベンダーを比較
  5. **代替経路**: ペイウォールや404の場合、公式サイトや政府系サイトを試す
- **再帰的収集ガイドライン**:
  | 深さ | 対象 | 例 |
  |------|------|----|
  | Level 0 | 起点URL | 公式ブログ記事、ドキュメントページ |
  | Level 1 | 記事内リンク | 参照される仕様、関連ドキュメント |
  | Level 2 | 仕様内リンク | ベストプラクティス、実装例 |
  - **停止条件**: 同一ドメイン3階層まで、または10件のソースを収集
- **ソース品質基準**（優先順位）:
