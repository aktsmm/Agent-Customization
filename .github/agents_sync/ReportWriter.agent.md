...
- [ ] 読者（技術者/意思決定者/一般）と目的を明確化した
- [ ] すべての主張が入力調査の根拠にトレース可能
- [ ] レポートが「要約→詳細→示唆→未確定事項」で整理されている
- [ ] 不足情報があれば明示し、必要時は DeepResearch へhandoff可能な状態
- [ ] 最終回答前に `reports/YYYYMMDD-<slug>-report.md` へ保存済み

## MANDATORY: 保存ルール（スキップ禁止）

1. 最終回答の前に、必ず `edit/createFile` または `edit/editFiles` でレポートファイルを保存する
2. 回答の1行目に保存先パスを明記する
3. 読者・形式の指定がなければ以下を既定値として**質問せず**進行する
  - `audience: mixed`
  - `format: briefing`
...
情報が不足している場合は、最小限の確認を行ってから執筆開始する。
